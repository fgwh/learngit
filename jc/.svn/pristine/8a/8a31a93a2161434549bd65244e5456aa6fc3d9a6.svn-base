...
public class ExcelServcie extends BaseService<BaseEntity> {
    ...
    public String validateRoGrayList(RoGrayList blackList, int count) {
        ...
        if (StringUtils.isEmpty(blackList.getCarNo())) {
            ...
            String reg = "^[A-Z_a-z]{1}[警京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{0,1}[A-Z_a-z_0-9]{4}[A-Z_a-z_0-9挂学警港澳]{1}$";
        ...
        if (flagNum == 0) {
            blackList.setPlateNum(blackList.getPlateNum().toUpperCase());
            blackList.setCarNo(blackList.getCarNo().toUpperCase());


//            Conditions conditions = new Conditions();
//            conditions.setPlateNum(blackList.getPlateNum());
//            conditions.setCarNo(blackList.getCarNo());
//            conditions.setVehPlate(blackList.getPlateNum() + blackList.getCarNo());
...
//            }
     ...
     /*   if (dicService.getItemsMap("vehClass").get(blackList.getVehClass() + "") == null) {
        ...
        if (StringUtils.isEmpty(blackList.getApplicant())) {
            resultSb.append("第" + (count + 3) + "行" + "第15列逃费总金额不能为空\\\n");
        }
...
//            }
     ...
     /*   if (dicService.getItemsMap("vehClass").get(blackList.getVehClass() + "") == null) {
        ...
        // 对逃费金额进行校验
        if (StringUtils.isEmpty(blackList.getApplicant())) {
            resultSb.append("第" + (count + 3) + "行" + "第10列逃费总金额不能为空\\\n");
        }
...
//        }
        ...
        // 对申请人进行校验
        if (StringUtils.isEmpty(blackList.getApplicant())) {
            resultSb.append("第" + (count + 3) + "行" + "第15列申请人不能为空\\\n");
        }
        // 对申请时间进行校验
        if (blackList.getApplicateTime() == null) {
            resultSb.append("第" + (count + 3) + "行" + "第16列申请时间不能为空或者格式不对\\\n");
        }

        // 对申请机构进行校验
        if (CacheUtil.getAllOrgCodeNameMap().get(blackList.getApplicateOrg()) == null) {
            resultSb.append("第" + (count + 3) + "行" + "第17列申请机构不能为空或者非法\\\n");
        }
        ...
        if (blackList.getInterceptOption() != 0 && blackList.getInterceptOption() != 1 && blackList.getInterceptOption() != 2 && blackList.getInterceptOption() != 3) {
            resultSb.append("第" + (count + 3) + "行" + "第18列拦截选项不能非法\\\n");
        }

        // 对拦截开始时间进行校验
        if (blackList.getStartDate() == null) {
            resultSb.append("第" + (count + 3) + "行" + "第19列拦截开始时间为空或者非法\\\n");
        }

        // 对拦截结束时间进行校验
        if (blackList.getEndDate() == null) {
            resultSb.append("第" + (count + 3) + "行" + "第20列拦截结束时间为空或者非法\\\n");
        }

        if (blackList.getStartDate() != null && blackList.getEndDate() == null) {
            if (blackList.getStartDate().compareTo(blackList.getEndDate()) > 0) {
                resultSb.append("第" + (count + 3) + "行" + "拦截开始时间不能大于结束时间\\\n");
            }
        }
...
//
//        boolean matches = "g123g2".matches("^[A-Z_a-z]{1}[警京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{0,1}[A-Z_a-z_0-9]{4}[A-Z_a-z_0-9挂学警港澳]{1}$");
//        System.out.println(matches);
        SimpleDateFormat format = new SimpleDateFormat("yyyy/MM/dd");
        Date parse = format.parse("2015/09/30");
        Date parse2 = format.parse("2015/04/30");

        int i = parse.compareTo(parse2);
        System.out.println(i);

        if (i > 0) {
//            System.out.println(“”“”“"非法时间");
            System.out.println("非法时间");
        }
