...
<form name="myForm" id="myForm" action="travelPath_list.do" method="post">
    ...
    <div class="widget widget-table">
        ...
        <div class="widget-bottom">
            ...
            <a class="btn btn-s-md btn-primary pull-right" id="print" onclick="savePath()"href="javascript:;"data-toggle="modal" data-target="#myModal4">生成行驶轨迹</a>
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody id="databody">
               	<c:forEach items="${list}" var="item" varStatus="status">
							<tr>
								<td>${item.enTime}</td>
								<td>${item.laneEnSerialNo}</td>
								<td>${item.enRoadId}</td>
								<td>${item.enStationId}</td>
								<td>${item.enLaneId}</td>
								<td>${item.enLaneType}</td>
								<td>${item.enVehiclePlate}</td>
								<td>${item.enVehiclePlate}</td>
								<td>${item.dealStatus}</td>
								<td>查看图像</td>
							</tr>
            ...
            <div class="modal-body">
                <table cellpadding="1" cellspacing="1" style="border:1px solid #ddd;"  class="pn-ftable dt-table">
	               	<c:forEach items="${list}" var="item" varStatus="status">
							<tr>
		                        <th>日期:</th>
		                        <td class="motaiTd"><font id="exTime2" class="motaiTdFont"  value="${item.exTime}"></font>${item.exTime}</td>
		                        <th>行驶轨迹:</th>
		                        <td class="motaiTd" style="display:none"><font id="realPathtxt" class="motaiTdFont" value="${item.realPath}"></font></td>
		                        <td class="motaiTd"><font id="realPath" class="motaiTdFont"></font>${item.realPath}</td>
	                   		</tr>
					</c:forEach>
                </table>
            </div>
   ...
   //导出Txt
 	...
 	function savePath() {
 		...
 		if (checkTimeLessThanWeek()) {
 			...
 			url = uri + "admin/travelPath_savePath.do";
 			$.post(url, params, reportExcelSucc, "json");
 		}
 	}

 	function reportExcelSucc(jsonTemp) {
 		var realPath = $("#realPathtxt").val(); 
 			var realPath1 = realPath;
			var LaneEnNo = $("#LaneEnNo").val();
			var ExCPCSnNo = $("#ExCPCSnNo").val();
		    var plateNum = $("#plateNum").val();
			var carNo = $("#carNo").val();
