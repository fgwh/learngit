...
<head>
    ...
    $(document).ready(function() { 
        ...
        // 添加年份，从2014年开始  
        for (i = 2014; i <= new Date().getFullYear(); i++) {  
            addOption(select_year, i, i);  
            // 默认选中
            if (i == yea ) { 
                select_year.options[i-2014+1].selected = true; 
            } 
        }  
        // 添加月份  
        for (i = 1; i <= 12; i++) {  
        ...
        // 添加天，先默认31天  
        var dayNum = getDaysInOneMonth($("#select_year").val(),$("#select_month").val());
         for (i = 1; i <= dayNum; i++) {  
