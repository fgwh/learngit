...
function save() {
	...
	{"dicItem.type" : $('[name="dicItem.type"]').val() , "dicItem.name" :  $('[name="dicItem.name"]').val() , "dicItem.value" :  $('[name="dicItem.value"]').val()},
	function(data){
		if(data=="one"){
			alert("有重复名字，请更改！");
...
<body>
     <form id="myForm" name="myForm" method="post" action="dicItem_save.do">
         <ul class="breadcrumb">
			<li><i class="icon-home icon-2x"></i></li>
			<li>当前位置：${currentPosition}&nbsp;>&nbsp;新增字典信息</li>
		 </ul>
			          ...
			          <tbody>
			                 <tr>
									<th> <span class="point">*</span>类别：</th>
			                         <td class="pn-fcontent"><input type="text" name="dicItem.type" id="type"
			                          placeholder="请输入字典类别" maxlength="25"/></td>	                         
			                 </tr>

							 <tr>
									<th><span class="point">*</span>名字：</th>
			                        <td class="pn-fcontent"><input type="text" name="dicItem.name" id="name"
			                          placeholder="请输入字典名字" maxlength="50"/></td>	
							 </tr>
			                 
			                 <tr>
									<th><span class="point">*</span>值  ：</th>
			                        <td class="pn-fcontent"><input type="text" name="dicItem.value" id="value"
			                          placeholder="请输入字典值" maxlength="50"/></td>	
			                 </tr>

							<tr>
									<th>内部分类  <span class="point"></span>：</th>
			                         <td class="pn-fcontent"><input type="text" name="dicItem.innerType" id="innerType"
			                          placeholder="请输入字典内部分类" maxlength="25"/></td>	
			                </tr>

                           <tr>
									<th><span class="point">*</span>内部序列号  ：</th>
			                         <td class="pn-fcontent"><input type="text" name="dicItem.innerOrder" id="innerOrder"
			                          onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')" placeholder="请输入字典内部序列号" maxlength="9"/></td>	
			                </tr>
			                         ...
			                         <th>备注 ：</th>
			                          <td class="pn-fcontent">
										  <input type="hidden" name="dicItem.timeDiff" value="0"/>
									  	  <textarea id="remark" name="dicItem.note" rows="4" cols="20" maxlength="120" ></textarea>
									  </td>
