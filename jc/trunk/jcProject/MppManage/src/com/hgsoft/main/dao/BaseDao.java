...
public class BaseDao<T extends Serializable> {
	...
	public List<?> findBySqlByPager(String sql, Pager pager) {
		SQLQuery query = getSession().createSQLQuery(sql);
		
		if (pager != null) {
			StringBuffer stringBf = new StringBuffer(sql);
			stringBf.replace(stringBf.lastIndexOf("order"), stringBf.length(), "");	
			String sql2= "select count(*) from ("+stringBf.toString()+")AP";
			System.out.println("查询countSQL："+sql);
			long a = Integer.parseInt(getSession().createSQLQuery(sql2).uniqueResult().toString());
