...
public class AbnormalCarDetailDao extends BaseDao<AbnormalCarDetail> {
	...
	// 获取流水
	public List<AbnormalCarDetail> getAbnorLiuShui(String plateNum, String auditStatus) {
		String sql = "select ac.id,ac.EnStationID,ac.EnTime,sd2.name as EnVehicleClass,ac.EnVehicleStatus,ac.EnVehiclePlate,ac.ExStationID,ac.ExTime,sd.name as ExVehicleClass,ac.ExVehicleStatus,"
				...
				+ "ac.ExRoadID,ac.SquadDate,ac.ExOperatorID,ac.AuditStatus FROM tb_AbnormalCarDeatils ac LEFT JOIN sys_dic sd ON ac.ExVehicleClass=sd.value "
		...
		if(!StringUtil.isTrimEmpty(auditStatus)){
			sql+= "  and ac.AuditStatus='"+auditStatus+"' ";
		}
	...
	public List<Object> getAbnormalCarDetail(String id) {
		   String sql="select  ac.id, so.orgName as EnStationID, ac.EnTime,sd2.name as EnVehicleClass,sd3.name as EnVehicleStatus, ac.EnVehiclePlate,so2.orgName as ExStationID, ac.ExTime,sd.name as ExVehicleClass, sd4.name as ExVehicleStatus,"
				+ " ac.ExVehiclePlate,ac.exVehIdentifyPlate,ac.DealStatus,ac.AxisGroupNum,ac.TotalWeight,ac.OverLoadWeight,ac.RealPath,ac.Miles,ac.OBUPlate,ac.OverLoadPercent,"
				+ " ac.ExceptionType,ac.ExVehicleFlag,ac.CashMoney,ac.EnNetRoadID,ac.ExNetRoadID,ac.LaneExSerialNo,ac.LaneEnSerialNo,ac.EnLaneID,ac.ExLaneID,ac.EnRoadID,"
				+ " ac.ExRoadID,ac.SquadDate,ac.ExOperatorID,ac.AuditStatus FROM tb_AbnormalCarDeatils ac LEFT JOIN sys_dic sd ON ac.ExVehicleClass=sd.value"
	...
	//标识点异常一  如果查询慢 可将in改成exists
	public List<Object[]> identifyPointOne(String year, String beginDate, String endDate,String roadCode) {
			String sql="select EnStationID,LaneEnSerialNo,EnTime,EnVehicleClass,EnVehicleStatus,EnVehiclePlate,ExStationID,ExTime,ExVehicleClass,ExVehicleStatus,ExVehiclePlate, ExVehIdentifyPlate,DealStatus,"
				+"AxisGroupNum,TotalWeight,OverLoadWeight,RealPath,Miles,ISNULL(OBUPlate,'')OBUPlate,(case when TotalWeightLimit='0' then 0 else ROUND(CAST(OverLoadWeight AS float)/TotalWeightLimit, 2) end ) as OverLoadPercent,"
				+"ExVehicleFlag,CashMoney,EnNetRoadID,ExNetRoadID,LaneExSerialNo,EnLaneID,ExLaneID,EnRoadID,ExRoadID,SquadDate,ExOperatorID  from tb_laneExList_"+year
			    +" where EnRoadID!='"+roadCode+"' and CashMoney>0 and (RealPath='' or DATALENGTH(realPath)<5) and ExVehiclePlate in (select t.ExVehiclePlate from tb_laneExList_"+year+"  t where dbo.vehPlate_legal(ExVehiclePlate)=0  and t.CashMoney>0 and (t.RealPath='' or DATALENGTH(t.realPath)<5)"
				+"  and  extime > convert(date,'"+beginDate+"')  and extime < convert(date,'"+endDate+"') group by t.ExVehiclePlate having count(*)>=2)";
	...
	public List<Object[]> identifyPointTwo(String year, String beginDate, String endDate) {
		String sql="select EnStationID,LaneEnSerialNo,EnTime,EnVehicleClass,EnVehicleStatus,EnVehiclePlate,ExStationID,ExTime,ExVehicleClass,ExVehicleStatus,ExVehiclePlate, ExVehIdentifyPlate,DealStatus,"
				+ "AxisGroupNum,TotalWeight,OverLoadWeight,RealPath,Miles,ISNULL(OBUPlate,'')OBUPlate,(case when TotalWeightLimit='0' then 0 else ROUND(CAST(OverLoadWeight AS float)/TotalWeightLimit, 2) end ) as OverLoadPercent,"
				+ "ExVehicleFlag,CashMoney,EnNetRoadID,ExNetRoadID,LaneExSerialNo,EnLaneID,ExLaneID,EnRoadID,ExRoadID,SquadDate,ExOperatorID  from tb_laneExList_"+year
				+"  b where  extime > convert(date,'"+beginDate+"') and extime < convert(date,'"+endDate+"')  and dbo.vehPlate_legal(ExVehiclePlate)=0 and  b.laneExSerialNo in (select a.LaneExSerialNo as LaneEnSerialNo from tb_ExPathPoint_"+year+"  a group by a.LaneExSerialNo,a.RealPathPoint having count(*) > 1) ";
		return (List<Object[]>) findBySql(sql,null);
	}
	//倒卡
	public List<Object[]> getPourCarException(String year, String beginDate, String endDate) {
		String sql = "IF OBJECT_ID('tempdb..#temp') is not null drop table #temp "
				...
				+ " group by ExVehiclePlate having COUNT(ExVehiclePlate) > =2 ) s left join tb_LaneExList_"+ year+"  t on s.ExVehiclePlate=t.ExVehiclePlate "
	...
	public List<Object[]> getETCVehTypeException(String year, String beginDate, String endDate) {
		String sql = "select  EnStationID,LaneEnSerialNo,EnTime,EnVehicleClass,EnVehicleStatus,EnVehiclePlate,ExStationID,ExTime,ExVehicleClass,"
				...
				+ year + " where dbo.vehPlate_legal(ExVehiclePlate)=0 and EnLaneType=3 and extime > convert(date,'" + beginDate
				+ "') and extime < convert(date,'" + endDate
	...
	public List<Object[]> getETCVehPlateException(String year, String beginDate, String endDate) {
		String sql = "select  EnStationID,LaneEnSerialNo,EnTime,EnVehicleClass,EnVehicleStatus,EnVehiclePlate,ExStationID,ExTime,ExVehicleClass,"
				...
				+ "') and EnLaneType=4 and dbo.vehPlate_legal(ExVehiclePlate)=0 and LEN(OBUPlate)>6 and LEN(ExVehIdentifyPlate)>6 and dbo.f_CompareSTR(ExVehIdentifyPlate,OBUPlate)>3 "
				+ "and  ExVehiclePlate in ("
				+ " select ExVehiclePlate from tb_LaneExList_"
				+ year + " where  extime > convert(date,'" + beginDate + "') and extime < convert(date,'" + endDate
				+ "') and ExLaneType=4 and dbo.vehPlate_legal(ExVehiclePlate)=0 and LEN(OBUPlate)>6 and LEN(ExVehIdentifyPlate)>6 "
				+"and dbo.f_CompareSTR(ExVehIdentifyPlate,OBUPlate)>3 "
				+ "group by ExVehiclePlate having COUNT(ExVehiclePlate)>1 ) ";
		return (List<Object[]>) findBySql(sql, null);
	}
	//保存到异常车辆表
	public void saveAbnorCarList(List<Object[]> list, String ExceptionType) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			abnormal.setEnNetRoadID(Short.valueOf(allDate[22].toString()));
			abnormal.setExNetRoadID(Short.valueOf(allDate[23].toString()));
			...
			abnormal.setAuditStatus((byte) 1);
	...
	public int deleteByReply() {
		String sql = "delete a from (select row_num = row_number() over(partition by ExRoadID,ExStationID,SquadDate,ExOperatorID,ExVehiclePlate,DealStatus,EnRoadID,EnStationID,LaneExSerialNo,ExceptionType order by id) from tb_AbnormalCarDeatils )a where row_num > 1";
		return getSession().createSQLQuery(sql).executeUpdate();
	}
}
